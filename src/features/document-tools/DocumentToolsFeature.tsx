...
export const DocumentToolsFeature = () => {
  ...
  const colorClass = activeToolData?.color === 'rose' ? 'text-rose-600 bg-rose-50 border-rose-200' : 'text-emerald-600 bg-emerald-50 border-emerald-200';
            ...
            {TOOLS_LIST.map((tool) => {
              ...
              return (
                <button
                  ...
                  className="group relative p-8 flex flex-col items-start h-full text-left rounded-3xl bg-white border border-slate-200/80 transition-all duration-300 overflow-hidden hover:border-indigo-300 hover:shadow-[0_20px_60px_-15px_rgba(79,70,229,0.15)] hover:-translate-y-1 z-10"
                >
                  ...
                  <div className={`p-4 rounded-2xl mb-6 transition-all duration-300 shadow-sm
                    ${tool.color === 'rose' ? 'bg-rose-50 text-rose-600 group-hover:bg-rose-500 group-hover:text-white' 
                  ...
                  <h3 className="text-2xl font-bold text-slate-900 group-hover:text-indigo-600 transition-colors duration-300 tracking-tight mb-2">
                    {tool.name}
                  </h3>
                  ...
                  <div className="mt-8 flex items-center gap-2 text-sm font-bold text-indigo-600 opacity-0 group-hover:opacity-100 transform -translate-x-4 group-hover:translate-x-0 transition-all duration-300">
          ...
          >
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-200">
              ...
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">{activeToolData?.name}</h2>
              </div>
            </div>
            ...
            <AnimatePresence>
              {error && (
                <motion.div 
                  ...
                  className="bg-rose-50 border border-rose-200 text-rose-700 px-5 py-4 rounded-2xl text-sm font-bold flex items-center justify-between shadow-sm"
                >
                  <div className="flex items-center gap-3">
                    <X className="w-5 h-5 bg-rose-200 text-rose-700 rounded-full p-0.5" />
                    {error}
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start h-full">
              ...
              <div className="liquid-glass-surface rounded-3xl shadow-sm border border-slate-200/80 p-6 flex flex-col gap-6 bg-white/60 backdrop-blur-xl">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {files.length > 0 && (
                    <button onClick={clearAll} className="text-xs font-bold text-rose-500 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 px-3 py-1.5 rounded-lg transition-colors">
                      Clear All
                    </button>
                  )}
                </div>
                ...
                <div 
                  className={`border-2 border-dashed rounded-2xl flex flex-col items-center justify-center text-center transition-all duration-300 cursor-pointer p-8 relative overflow-hidden group
                    ${files.length >= (isMultiFile ? MAX_FILES : 1) ? 'opacity-50 pointer-events-none bg-slate-50 border-slate-200' : 'hover:bg-indigo-50/50 border-slate-300 hover:border-indigo-300'}`}
                ...
                >
                  ...
                  <h3 className="text-base font-bold text-slate-900 mb-1">Click or drag to upload</h3>
                  ...
                  <div className="flex flex-col gap-3">
                    {files.map((file, idx) => (
                      <div key={idx} className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-2xl shadow-sm">
                        ...
                        <button onClick={() => removeFile(idx)} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-colors shrink-0">
                          <X className="w-5 h-5" />
                        </button>
                      </div>
                    ))}
              ...
              <div className="liquid-glass-surface rounded-3xl shadow-sm border border-slate-200/80 p-6 flex flex-col bg-white/60 backdrop-blur-xl h-full min-h-[400px]">
                <h3 className="font-extrabold text-slate-900 flex items-center gap-2 text-base mb-6">
                  <FileOutput className="w-5 h-5 text-indigo-500" />
                  Processed Result
                </h3>
                
                <div className="flex-1 flex flex-col relative border-2 border-dashed border-slate-200 rounded-2xl overflow-hidden bg-slate-50/50">
                  <AnimatePresence mode="wait">
                    {!resultUrl ? (
                      <motion.div 
                        key="empty"
                      ...
                      >
                        ...
                        <h4 className="font-bold text-slate-500 mb-2">No Output Yet</h4>
                      ...
                      >
                        <div className="w-24 h-24 bg-emerald-50 rounded-full flex items-center justify-center mb-6 border-4 border-white shadow-lg">
                          <CheckCircle2 className="w-12 h-12 text-emerald-500" />
                        </div>
                        
                        <h3 className="font-extrabold text-slate-900 text-xl text-center break-all mb-2">
                          {resultFileName}
                        </h3>
                        
                        {resultSize && (
                          <p className="text-sm font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full mb-8">
                        ...
                        <a 
                          ...
                          className="w-full flex items-center justify-center gap-3 bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-4 rounded-2xl text-base font-bold transition-all shadow-[0_8px_20px_rgba(16,185,129,0.2)] active:scale-[0.98]"
                        >
